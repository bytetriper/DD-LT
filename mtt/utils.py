...
def instantiate_from_config(config: DictConfig):
    ...
    if "cache_dir" in params and os.path.exists(params["cache_dir"]):
        return datasets.load_from_disk(params["cache_dir"])

    # return get_obj_from_str(config["target"])(**config.get("params", dict()))
    return target(**config.get("params", dict()))
...
def get_dataset(dataset, data_path, batch_size=1, subset="imagenette", args=None):
    ...
    if dataset == 'CIFAR10':
        ...
        # r-10 mean:
        # mean = [0.0167, 0.0517, 0.0984]
        # std = [1.2241, 1.2280, 1.3333]
        
        # r-100 mean:
        # mean = [0.0409, 0.1056, 0.2288]
        # std = [1.2338, 1.2407, 1.3576]
        ...
        dst_train, dst_test, channel, im_size = get_dataset_from_config('./config/Cifar10lt.yaml')
        
        def apply_transform(example):
            example['img'] = transform(example['img'])
            return example
        dst_train = dst_train.map(apply_transform)
        dst_test = dst_test.map(apply_transform)
        # dst_train = datasets.CIFAR10(data_path, train=True, download=True, transform=transform) # no augmentation
        # dst_test = datasets.CIFAR10(data_path, train=False, download=True, transform=transform)
        # class_names = dst_train.classes
        class_names = set()
        for example in dst_train:
            label = example['label']
            # class_name = dst_train.features["label"]
...
def evaluate_synset(it_eval, net, images_train, labels_train, testloader, args, return_loss=False, texture=False):
    ...
    for ep in tqdm.tqdm(range(Epoch+1)):
        ...
        if ep == Epoch:
            with torch.no_grad():
                loss_test, acc_test, each_class_acc = epoch('test', testloader, net, optimizer, criterion, args, aug=False)
