...
def main(args):
    ...
    for it in range(0, args.Iteration+1):
        ...
        if it == 0:
            load_dir = os.path.join(".", "logged_files", args.dataset, 'decent-dream-2')
            # image_syn = torch.load(os.path.join(load_dir, "images_best.pt".format(it)))
            # label_syn = torch.load(os.path.join(load_dir, "labels_best.pt".format(it)))
            image_syn = images_all
            label_syn = labels_all
